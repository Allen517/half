...
class _LINE(object):
    ...
    def batch_iter(self):
        ...
        edges = [(self.look_up[v], self.look_up[nd]) for v,n_nds in self.g.G.iteritems() for nd in n_nds.keys()]
        data_size = self.g.num_of_edges
    ...
    def _gen_sampling_table(self):
        ...
        for v in self.g.G.keys():
            for nd in self.g.G[v].keys():
                self.node_degree[look_up[v]] += self.g.G[v][nd]
        ...
        total_sum = sum([self.g.G[v][nd] for v,n_nds in self.g.G.iteritems() for nd in n_nds.keys()])
        norm_prob = [self.g.G[v][nd]*data_size/total_sum for v,n_nds in self.g.G.iteritems() \
                        for nd in n_nds.keys()]
        num_small_block = 0
        num_large_block = 0
        cur_small_block = 0
        cur_large_block = 0
